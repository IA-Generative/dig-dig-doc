...
def _make_dossier_response(
    *, with_summary: bool = True, with_documents: bool = True
) -> dict:
    dossier = {
        "id": "dossier-1",
        "analyse_id": None,
    ...
    if with_summary:
        dossier["summary"] = {
            "content": "Dossier contenant une CNI et un justificatif de domicile."
        }
    if with_documents:
        dossier["documents"] = [
            {
...
def test_suggest_dossier_analyse_deposits_suggestions(monkeypatch) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.method == "PUT" and path.endswith("/suggestion-status"):
            return httpx.Response(
                200, json={"suggestion_status": json.loads(request.content)["status"]}
            )
    ...
    fake_result = SuggestionResult(
        suggestions=[
            AnalyseSuggestion(
                analyse_id="analyse-cni", score=0.95, rationale="CNI présente"
            ),
            AnalyseSuggestion(
    ...
    monkeypatch.setattr(
        suggestion_mod, "suggest_analyses", lambda **kwargs: fake_result
    )
...
def test_suggest_dossier_analyse_no_analyses_deposits_empty(monkeypatch) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if path.endswith("/analyses"):
            return httpx.Response(
                200, json={"items": [], "total": 0, "page": 1, "page_size": 100}
            )
        if request.method == "POST" and path.endswith("/suggestions"):
            deposited.append(json.loads(request.content))
            return httpx.Response(200, json={"suggestion_status": "terminé"})
