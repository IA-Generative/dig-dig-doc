...
class DossierRepository:
    ...
    def _conversation_query(self):
        ...
        feedback_load = (
            ...
            .selectinload(Feedback.reason_rows)
        )
        return (
            select(Conversation)
            .options(
    ...
    async def list_conversations_paginated(
        ...
        base = self._conversation_query().where(
            Conversation.dossier_id == dossier_id, Conversation.user_id == user_id
        )
        count_query = (
            select(func.count())
        ...
        result = await self.db.execute(
            base.order_by(Conversation.created_at)
            .limit(page_size)
            .offset((page - 1) * page_size)
        )
        return result.scalars().all(), total or 0
    ...
    async def list_conversations_for_user_paginated(
        ...
        last_msg = (
            select(
                Message.conversation_id, func.max(Message.created_at).label("last_at")
            )
    ...
    async def update_conversation_model(
        self, conversation: Conversation, model: str | None
    ) -> Conversation:
    ...
    async def set_feedback(
        ...
        await self.db.execute(
            FeedbackReason.__table__.delete().where(
                FeedbackReason.feedback_id == existing.id
            )
        )
    ...
    async def add_chat_event(
        self, conversation_id: uuid.UUID, kind: ChatEventKind, data: dict
    ) -> ChatEvent:
    ...
    async def list_chat_events(
        ...
        if after_id is not None:
            ...
            sub = (
                select(ChatEvent.created_at)
                .where(ChatEvent.id == after_id)
                .scalar_subquery()
            )
    ...
    async def delete_chat_events(self, conversation_id: uuid.UUID) -> None:
        ...
        await self.db.execute(
            ChatEvent.__table__.delete().where(
